...
do
    ...
    sleep 1
    echo "------------------------>depth:${td}"
    ssh ${server_host}  numactl -N 0 ib_read_bw -s 4096 -q ${td} -d mlx5_0 -p 999  > "server_"${td}".log" &
    sleep 1
    numactl -N 0 ib_read_bw -s 4096 -q ${td} -F 192.168.0.1 -d mlx5_0 -p 999  > "client_"${td}".log" &
    sleep 5
