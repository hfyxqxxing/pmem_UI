...
do
    ...
    echo -e "\e[33mnumactl -m ${BIND_SOCKET} taskset -c $core_config  $REDIS_PATH/redis-server --port ${port} --maxmemory ${max_memory} --nvm-maxcapacity ${nvm_max_capacity} --nvm-dir /mnt/pmem${BIND_SOCKET}/ --nvm-threshold ${nvm_threshold} --maxmemory-policy allkeys-lru --dir ${DIR_PATH} --appendfilename ${port}.aof --appendonly yes --appendfsync always --pointer-based-aof yes --auto-aof-rewrite-percentage 0  >/dev/null 2>&1  &\e[0m"
    numactl -m ${BIND_SOCKET} taskset -c $core_config  $REDIS_PATH/redis-server --port ${port} --maxmemory ${max_memory} --nvm-maxcapacity ${nvm_max_capacity} --nvm-dir /mnt/pmem${BIND_SOCKET}/ --nvm-threshold ${nvm_threshold} --maxmemory-policy allkeys-lru --dir ${DIR_PATH} --appendfilename ${port}.aof --appendonly yes --appendfsync always --pointer-based-aof yes --auto-aof-rewrite-percentage 0  >/dev/null 2>&1  &
    fi
done
