...
do
   ...
   echo -e "\e[33mnumactl -m ${BIND_SOCKET} taskset -c $core_config $REDIS_PATH/redis-server --port ${port} --maxmemory ${max_memory} --maxmemory-policy allkeys-lru --dir ${DIR_PATH} --save '' --appendonly yes --appendfsync always --auto-aof-rewrite-percentage 0  --appendfilename ${port}.aof  >/dev/null 2>&1 &\e[0m"
   numactl -m ${BIND_SOCKET} taskset -c $core_config $REDIS_PATH/redis-server --port ${port} --maxmemory ${max_memory} --maxmemory-policy allkeys-lru --dir ${DIR_PATH} --save '' --auto-aof-rewrite-percentage 0 --appendonly yes --appendfsync always --appendfilename ${port}.aof  >/dev/null 2>&1 &
   fi
